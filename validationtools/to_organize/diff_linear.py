...
from validation_tools import raster_comparison
...
def compare_linregress(image_path1,image_path2, output_folder):
    ...
    for filename in os.listdir(image_path1):
        if filename.endswith('.tif') or filename.endswith('.jp2'):
            i1 = filename
            print(i1)
...
def img_linregress(image_path1, image_path2, output_dir, out_file=None):
    """Perform image linear regression.
        Args:
    ...
    ds1, ds2, _ = raster_comparison.raster_intersection(image_path1, image_path2)

    bandref = np.array(ds1.GetRasterBand(1).ReadAsArray())
    bandtar = np.array(ds2.GetRasterBand(1).ReadAsArray())
    #Convert np array to float
    bandref = bandref.astype(float)
    bandtar = bandtar.astype(float)
    #set NaN == -9999
    ...
    stats = 'slope:{} \nintercept:{} \nr_value:{} \np_value:{} \nstd_error:{} \nr-squared:{}'.format(slope, intercept, r_value, p_value, std_err, r2)
    print(stats)
    ...
    if out_file is None:
        out_file = os.path.join(output_dir, (os.path.basename(image_path1)[:-4] + '__' + os.path.basename(image_path2)[:-4]))
    os.chdir(output_dir)
    ...
    ### Draw Line given 2 extreme points
    x2 = np.array([0, 10000])
    plt.plot(x2, x2, color = ('#808080'), ls='dashed', linewidth=1)
    plt.plot(x2, slope * x2 + intercept, '--k', linewidth=1)
    plt.xlabel(x_label, fontsize=14)
    plt.ylabel(y_label, fontsize=14)
    plt.xlabel(os.path.basename(image_path1))
    plt.ylabel(os.path.basename(image_path2))
    plt.colorbar()
    plt.savefig(out_file +'.png', dpi=300, bbox_inches='tight')
    plt.close(fig=None)
